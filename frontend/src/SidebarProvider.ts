...
export class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  _context: vscode.ExtensionContext; // Add context for state storage
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      ...
      switch (message.command) {
        ...
        case 'getStoredSession':
          ...
          const taskId = this._context.globalState.get<string>('triage-current-task');
          const currentStep = this._context.globalState.get<number>('triage-current-step') || 1;
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    // Get icons
    const pmIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, "media", "icons", "pm-icon.svg")
    );
    const architectIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, "media", "icons", "architect-icon.svg")
    );
    const securityIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, "media", "icons", "security-icon.svg")
    );
    const testIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, "media", "icons", "test-icon.svg")
    );
